...
class Bits(int):
    ...
    def compose(cls, *flags: bool) -> Bits:
        ...
        """
        return cls(reduce(or_, (bit << i for i, bit in enumerate(flags) if bit), cls()))
    ...
    def extract(self, mask: str, *, sep: str = ' ') -> Tuple[int, ...]:
        ...
        Each group of adjacent digits holds position of a separate number to be extracted
    ...
    def extract2(self, mask: str, *, sep: str = ' ') -> List[int, ...]:
        """
        A 20% slower implementation with same functionality
        This variant also returns a list instead of tuple
        """
    ...
    def pack(self, mask: str, *nums: int, sep: str = ' ') -> Bits:
        """
        Insert values specified in 'nums' into current Bits object
        Functionally is the inverse of .extract() method
            on specified positions denoted by 'mask'
        Mask consists of 3 types of markers:
            • index marker - digits from 0 to <len of 'nums' argument>
            • delimiter - any character (except for digit or specified separator character)
            • separator - character specified by 'sep' (could not be another marker character)
        Each group of adjacent digits holds position of a separate number to be inserted
